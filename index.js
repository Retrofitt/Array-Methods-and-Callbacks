...
function getFinals(data) {
   const finalist = data.filter(item => item.Stage === 'Final');
...
function getWinnersByYear(array, getYearsCb, getWinnersCb) {
    const yearsCb = getYearsCb(array, getFinals);
    const winnersCb = getWinnersCb(array, getFinals);
    const winningYear = [];
    yearsCb.forEach((item, i) => {
        const item2 = winnersCb[i];
        winningYear.push(`In ${item}, ${item2} won the world cup!`)
    })
    return winningYear   
}
...
function getAverageGoals(getFinalsCb){
    ...
    const avgGoal = finals.reduce((acc, item) => {
       return acc + item['Home Team Goals'] + item['Away Team Goals'];
    },0);
    const totalAvg = (avgGoal/finals.length).toFixed(2);
    return totalAvg
}
